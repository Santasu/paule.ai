...
export default async function handler(req, res){
  try{
    ...
    const { message = '', models = '', max_tokens = 1024, search = 'auto' } = req.body || {};
    ...
    // ✅ OpenAI per responses API (be SSE, tinkama JSON režimui)
    async function askOpenAI(model){
      const r = await fetch('https://api.openai.com/v1/responses', {
        ...
        body: JSON.stringify({
          ...
          max_output_tokens: Number(max_tokens)||1024
        })
      });
      const j = await r.json().catch(()=>null);
      if (!r.ok) return { model, error: j?.error?.message || `HTTP ${r.status}` };
    ...
    // — Together
    async function askTogether(model){
    ...
    for (const m of list){
      try{
        ...
        if (isTogether(m))    { answers.push(await askTogether(m));    continue; }
