export default async function handler(req, res) {
  ...
  // pažymėk kurie tikrai turi SSE tavo backend’e
  const sseSet = new Set(['llama','deepseek']);

  const items = Object.keys(frontToBack)
    .filter(k=>k!=='paule' && k!=='augam-auto')
    .map(k=>({ id:k, back:frontToBack[k], name:friendly[k]||k, icon:icon[k]||icon['auto'], sse:sseSet.has(k) }));

  res.setHeader('Access-Control-Allow-Origin', '*');
  res.setHeader('Cache-Control', 'no-store');
  res.status(200).json({ ok:true, items, front_to_back:frontToBack, sse_front:Array.from(sseSet) });
}
