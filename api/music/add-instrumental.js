module.exports = async (req, res) => {
  try {
    ...
    const body = Object.assign({}, req.body || {}, { callBackUrl: (req.body && req.body.callBackUrl) || (base ? `${base}/api/music/callback` : undefined) });

    if (!SUNO_API_KEY) return res.status(200).json({ ok:false, error:'SUNO_API_KEY missing' });

    const r = await fetch(`${SUNO_API_BASE}/generate/add-instrumental`, {
      method:'POST',
      headers:{ 'Authorization':`Bearer ${SUNO_API_KEY}`, 'Content-Type':'application/json' },
      body: JSON.stringify(body)
    });
    const j = await r.json().catch(()=> ({}));
    if (!r.ok || j.code !== 200) return res.status(200).json({ ok:false, error: j.msg || 'add-instrumental failed', raw:j });
