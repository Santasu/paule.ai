...
(function () {
  ...
  // Modeliai, kuriems NENAUDOJAM SSE (traktuojam kaip JSON "once")
  ...
  function canonicalFrontId(id){
    if (!id) return 'auto';
    let s = lc(id);
    // Jei atėjo BACK id – konvertuojam į front
    if (!FRONT_TO_BACK[s] && BACK_TO_FRONT[id]) {
      s = BACK_TO_FRONT[id];
    }
